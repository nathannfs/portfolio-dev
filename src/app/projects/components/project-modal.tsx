  ...
  return (
    <Modal
      ...
      description={initialData
        ? 'Edite o projeto abaixo.'
        : 'Adicione um novo projeto preenchendo os campos abaixo.'}
    >
          ...
          <ul className="flex flex-wrap gap-2">
            {techs.map((tech, idx) => (
              <li key={idx} className="flex items-center gap-1 rounded bg-zinc-100 px-2 py-1 text-sm dark:bg-zinc-800">
                {tech}
                <button type="button" onClick={() => removeFromList(techs, setTechs, idx)} aria-label="Remover tecnologia">
        ...
        <div className="flex flex-col gap-1">
          ...
          <ul className="flex flex-wrap gap-2">
            {features.map((item, idx) => (
              <li key={idx} className="flex items-center gap-1 rounded bg-zinc-100 px-2 py-1 text-sm dark:bg-zinc-800">
                {item}
                <button type="button" onClick={() => removeFromList(features, setFeatures, idx)} aria-label="Remover funcionalidade">
                  <X className="size-3 ml-1" />
                </button>
              </li>
            ))}
        ...
        <div className="flex flex-col gap-1">
          ...
          <ul className="flex flex-wrap gap-2">
            {challenges.map((item, idx) => (
              <li key={idx} className="flex items-center gap-1 rounded bg-zinc-100 px-2 py-1 text-sm dark:bg-zinc-800">
                {item}
                <button type="button" onClick={() => removeFromList(challenges, setChallenges, idx)} aria-label="Remover desafio">
                  <X className="size-3 ml-1" />
                </button>
              </li>
            ))}
        ...
        <div className="flex flex-col gap-1">
          ...
          <ul className="flex flex-wrap gap-2">
            {learnings.map((item, idx) => (
              <li key={idx} className="flex items-center gap-1 rounded bg-zinc-100 px-2 py-1 text-sm dark:bg-zinc-800">
                {item}
                <button type="button" onClick={() => removeFromList(learnings, setLearnings, idx)} aria-label="Remover aprendizado">
                  <X className="size-3 ml-1" />
                </button>
              </li>
            ))}
          ...
          <div className="flex gap-2 mt-1">
            <Input.Root className="flex-1">
              <Input.Control
                placeholder="Adicionar aprendizado"
                value={newLearning}
